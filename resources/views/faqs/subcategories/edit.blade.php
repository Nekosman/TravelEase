    ...
    <div class="form-wrapper"> <!-- Wrapper untuk mengontrol layout secara fleksibel -->
        <div class="form-container">
            <div class="card shadow-lg"> <!-- Tambahkan shadow -->
                <div class="card-header text-white" style="background-color: #366389;">edit FAQ subcategory</div>

                <div class="card-body">
                    <form method="POST" action="{{ route('faqSubCategory.update', $faqSubCategory->id) }}">
                        @method('PUT')
                        @csrf
                        <div class="mb-3">
                            <label for="question" class="form-label">Name</label>
                            <input type="text" class="form-control @error('question') is-invalid @enderror"
                                id="name" name="name" value="{{ old('name', $faqSubCategory->name) }}">
                            @error('name')
                            ...
                            <select class="form-select @error('faq_category_id') is-invalid @enderror" id="faq_category_id"
                                ...
                                @foreach ($faqcategories as $category)
                                    <option value="{{ $category->id }}"
                                        {{ old('faq_category_id', $faqSubCategory->faq_category_id) == $category->id ? 'selected' : '' }}>
                                        {{ $category->name }}
                                    </option>
                                @endforeach
                            </select>
